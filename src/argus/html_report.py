...
def _health_section(results: list[ScanResult], failures: list[tuple[str, Exception]]) -> str:
    cards = []
    ...
    return (
        ...
        f'<span class="count">{len(results)} ok &middot; {len(failures)} failed</span></h2>'
...
{watch_section}
{categories}
{_health_section(results, failures)}
